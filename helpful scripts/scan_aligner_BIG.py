...
def detect_aruco_centers(img_bgr: np.ndarray, dict_name: str = "DICT_4X4_50") -> Dict[int, Tuple[float, float]]:
    ...
    “This function takes an OpenCV image (numpy.ndarray)
    and an optional string specifying which ArUco dictionary to use,
    and it returns a dictionary mapping marker IDs (integers)
    to their center coordinates (floats).”"""
...
def parse_args():
    ...
    ap.add_argument("--aruco-dict", type=str, default="DICT_4X4_50", help="ArUco dictionary. Default: 4x4_50")
    ap.add_argument("--aruco-min", type=int, default=4, help="Minimum ArUco markers to proceed with ArUco alignment. Default: 4")
    ap.add_argument("--aruco-length", type=float, default=0.04, help="ArUco marker side length in meters. Default: 0.04")
    ap.add_argument("--aruco-margin", type=float, default=0.005, help="ArUco marker margin in meters. Default: 0.005")
    # DPI
    ap.add_argument("--no-dpi-check", action="store_true", help="Disable DPI consistency check between template and scans.")
    # Feature params
...
#we should now have a list of tuples (src_path, page_idx, scan_bgr) for each page in the input pdf(s)
#at this point we could split into ARUCO and non-ARUCO pages

    if args.aruco:
        print("[info] ArUco-based alignment enabled.", file=sys.stderr)
        aruco_fpar = FeatureParams(tiles_x=6, tiles_y=8, topk_per_tile=150, orb_nfeatures=3000, orb_fast_threshold=12)
        aruco_epar = EstParams(method=args.method, ransac_thresh=args.ransac_thresh, max_iters=args.max_iters,
                              confidence=args.confidence, ecc_levels=args.ecc_levels, ecc_iters=args.ecc_iters,
                              ecc_eps=args.ecc_eps, use_ecc=(args.ecc))
        non_aruco_pages = []
        for (src_path, page_idx, scan_bgr) in all_scan_pages:
            try:
                warped_a, H_a, detected_ids = align_with_aruco(scan_bgr, template_bgr, dict_name=args.aruco_dict,
                                                               min_markers=args.aruco_min, ransac=args.ransac_thresh)
                if warped_a is not None and H_a is not None and len(detected_ids) >= args.aruco_min:
                    metrics = {"res_median": float("nan"), "res_p95": float("nan"), "res_br_p95": float("nan"),
                               "n_inliers": len(detected_ids), "mode": "aruco", "detected_ids": detected_ids}
                    aligned_pages.append(warped_a)
                    if args.out_dir:
                        base = os.path.splitext(os.path.basename(src_path))[0]
                        fn = os.path.join(args.out_dir, f"{args.prefix}_{base}_p{page_idx:03d}.png")
                        _imwrite(fn, warped_a)
                    row = {
                        "source": os.path.basename(src_path),
                        "page": page_idx,
                        "mode": metrics.get("mode","aruco"),
                        "res_median": "nan",
                        "res_p95": "nan",
                        "res_br_p95": "nan",
                    ...
                    print(f"[info] {src_path} p{page_idx}: ArUco alignment failed or insufficient markers ({len(detected_ids)}). Falling back.", file=sys.stderr)
                ...
                non_aruco_pages.append((src_path, page_idx, scan_bgr))
        all_scan_pages = non_aruco_pages
        if not all_scan_pages:
    ...
    # Now process remaining pages with feature-based alignment with guardrails
