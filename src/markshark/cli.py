...
def align(
    ...
    image_format: str = typer.Option(RENDER_DEFAULTS.image_format, "--image-format", help="png|jpg for intermediates"),
    jpeg_quality: int = typer.Option(RENDER_DEFAULTS.jpeg_quality, "--jpeg-quality", help="JPEG quality if jpg"),
    keep_intermediates: bool = typer.Option(RENDER_DEFAULTS.keep_intermediates, "--keep-intermediates/--no-keep-intermediates", help="Keep debug intermediates"),
    ...
    confidence: float = typer.Option(EST_DEFAULTS.confidence, "--confidence", help="RANSAC/USAC confidence (0-1)"),
    use_ecc: bool = typer.Option(EST_DEFAULTS.use_ecc, "--use-ecc/--no-use-ecc", help="Enable ECC refinement"),
    ecc_levels: int = typer.Option(EST_DEFAULTS.ecc_levels, "--ecc-levels", help="ECC pyramid levels"),
    ecc_max_iters: int = typer.Option(EST_DEFAULTS.ecc_max_iters, "--ecc-max-iters", help="ECC iterations per level"),
    ...
    mutual_check: bool = typer.Option(MATCH_DEFAULTS.mutual_check, "--mutual-check/--no-mutual-check", help="Require mutual A↔B matches"),
    use_flann: bool = typer.Option(MATCH_DEFAULTS.use_flann, "--use-flann/--no-use-flann", help="Use FLANN vs brute-force"),
    max_matches: int = typer.Option(MATCH_DEFAULTS.max_matches, "--max-matches", help="Hard cap on total matches"),
    dict_name: str = typer.Option(ALIGN_DEFAULTS.dict_name, "--dict-name", help="ArUco dictionary"),
    use_aruco: bool = typer.Option(ALIGN_DEFAULTS.use_aruco, "--aruco/--no-aruco", help="Enable ArUco coarse align"),
    export_pngs: bool = typer.Option(ALIGN_DEFAULTS.export_pngs, "--export-pngs/--no-export-pngs", help="Export aligned PNGs"),
    overwrite: bool = typer.Option(ALIGN_DEFAULTS.overwrite, "--overwrite/--no-overwrite", help="Overwrite outputs"),
    verbose: bool = typer.Option(ALIGN_DEFAULTS.verbose, "--verbose/--quiet", help="Verbose logging"),
