...
class AlignOnlyPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.bubblemap_selector = FileSelector(
            "Bubblemap YAML (optional):",
            "YAML files (*.yaml *.yml)",
            "Override bubblemap for fast alignment...",
        )
        self.bubblemap_selector.setToolTip(
            "Provide a bubblemap to enable fast (coarse-to-fine) alignment mode."
        )
        files_layout.addWidget(self.bubblemap_selector)
    ...
    def _run_align(self):
        ...
        # Bubblemap: explicit override or from template
        if self.bubblemap_selector.exists():
            args += ["--bubblemap", self.bubblemap_selector.path()]
        else:
